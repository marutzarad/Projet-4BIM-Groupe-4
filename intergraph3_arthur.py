...
import random
import pandas as pd
from portrait_robot_complet import vae, device, decode_latents_to_images, generate_initial_population, get_latents_from_images, attribute_directions, force_attributes_on_latents
...
def open_third_window():
    ...
    third_window.geometry("800x800")
    third_window.configure(bg="#FFEBCD")

    frame = tk.Frame(third_window, bg="#FFEBCD")
    frame.pack()

    selected_imgs, selected_attrs = get_user_choices()

    # Prétraitement des images sélectionnées
    ...
    # Encoder → générer enfants → forcer attributs → décoder
    ...
    children_latents = force_attributes_on_latents(children_latents, selected_attrs)
    ...
    img_display = ImageTk.PhotoImage(grid_img.resize((700, 700)))
    img_label = tk.Label(frame, image=img_display)
    img_label.image = img_display  # Garde une référence
    img_label.pack()
