...
import random
import pandas as pd
from portrait_robot_13h33 import *
from torchvision import transforms
from PIL import Image
import torch
from torchvision.utils import make_grid
from torchvision.transforms.functional import to_pil_image
from portrait_robot_13h33 import attribute_directions, compute_attribute_directions_from_csv
...
def image_selection(img_name, var):
    ...
    img_full_path = os.path.join("C:/Users/adele/Documents/4A local/S2/Projet_WEB/data_celebia/Img-20250407T213121Z-001/Img/img_align_celeba/img_align_celeba", img_name)
...
def open_fourth_window(parent_latents):
    ...
    fourth_window.title("Nouvelle génération")
    fourth_window.geometry("850x800")
    ...
    def next_generation():
        ...
        if selected is not None and len(selected) == 1:
            ...
            open_final_result(images[[i for i, v in enumerate(check_vars) if v.get() == 1][0]])
            return
        selected = get_selected_children()
    ...
    def go_back():
        if generation_history:
            previous_latents = generation_history.pop()
            fourth_window.destroy()
            open_fourth_window(previous_latents)

    # Relancer avec sélection (déjà existant)
    button_another = tk.Button(fourth_window, text="Encore une génération !", bg="#ad7e66", command=next_generation, padx=10, pady=5)
    button_another.pack(pady=10)

    # Regénérer les enfants avec les mêmes parents (nouveau)
    button_children = tk.Button(fourth_window, text="Regénérer les enfants (même parents)", bg="#ecd7c6",
    ...
    button_back = tk.Button(fourth_window, text="Retour à la génération précédente", bg="#cf7928", command=go_back, padx=10, pady=5)
...
def open_main_window():
    ...
    photos_directory = os.path.expanduser("C:/Users/adele/Documents/4A local/S2/Projet_WEB/data_celebia/Img-20250407T213121Z-001/Img/img_align_celeba/img_align_celeba")
...
def open_final_result(image_tensor):
    ...
    final_window.geometry("400x500")
    final_window.configure(bg="#ecd7c6")
    final_description = ("And here is the generated image by VisaGen! \n"
                         "We hope that the character created corresponds to your wishes and that its was an easy and enjoyable experience!\n"
                         "Come see us again if you have another project in mind!\n")
    
    final_label = tk.Label(final_window, text=final_description, font=("Century Gothic", 12), bg="#ecd7c6")
