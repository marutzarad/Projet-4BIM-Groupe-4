...
import random
import pandas as pd
from portrait_robot_12h49 import *
from torchvision import transforms
from PIL import Image
import torch
from torchvision.utils import make_grid
from torchvision.transforms.functional import to_pil_image
from portrait_robot_12h49 import attribute_directions, compute_attribute_directions_from_csv
...
def open_fourth_window(parent_latents):
    ...
    fourth_window.geometry("850x900")
    ...
    bouton_selec = tk.Button(fourth_window, text="Encore une génération !", bg="#ad7e66", command=next_generation, padx=10, pady=5)
    bouton_selec.pack(pady=10)

    # Regénérer les enfants avec les mêmes parents (nouveau)
    bouton_regenere = tk.Button(fourth_window, text="Regénérer les enfants (même parents)", bg="#ecd7c6",
              command=lambda: (fourth_window.destroy(), open_fourth_window(parent_latents)),
              padx=10, pady=5)
    bouton_regenere.pack(pady=5)

    # Retour à la génération précédente
    bouton_precedent = tk.Button(fourth_window, text="Retour à la génération précédente", bg="#cf7928", command=go_back, padx=10, pady=5)
    bouton_precedent.pack(pady=5)
...
def open_main_window():
    ...
    attributes_modified.remove("Wearing Hat")
